...
const FIELD_CONSTRAINTS = {
	...
	seo_description: {
		min: 140,
		max: 160,
		label: __( 'SEO Description', 'meowseo' ),
	},
	focus_keyword: {
		min: 1,
		max: 100,
	...
	og_description: {
		min: 100,
		max: 200,
		label: __( 'OG Description', 'meowseo' ),
	},
...
export function PreviewPanel( {
	...
	const renderField = ( fieldName, value ) => {
		...
		if ( constraint ) {
			if ( constraint.min > 0 ) {
				ariaDescription = __(
					`Character count: ${ length } of ${ constraint.max } maximum, minimum ${ constraint.min } required`,
					'meowseo'
				);
			} else {
				ariaDescription = __(
					`Character count: ${ length } of ${ constraint.max } maximum`,
					'meowseo'
				);
			}
		}
		...
		return (
			<div
				key={ fieldName }
				className={ `meowseo-preview-field ${ getFieldClass(
					fieldName,
					value
				) }` }
			>
						...
						help={
							...
							`${
								constraint.min > 0
									...
									: ''
							}Maximum: ${ constraint.max } characters`
						}
					/>
				) : (
					<TextControl
	...
	return (
		<div
			className="meowseo-preview-panel"
			role="region"
			aria-label={ __( 'Generated content preview', 'meowseo' ) }
		>
			...
			<div
				className="meowseo-preview-fields"
				role="group"
				aria-label={ __( 'Editable generated fields', 'meowseo' ) }
			>
				{ Object.keys( editedContent ).map( ( fieldName ) => {
					// Skip non-text fields
					if (
						[ 'image_url', 'provider', 'is_fallback' ].includes(
							fieldName
						)
					) {
						return null;
