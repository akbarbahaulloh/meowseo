...
jest.mock('../components/PreviewPanel', () => ({
	PreviewPanel: ({ content, onApply, onCancel, isApplying, provider }) => (
		<div data-testid="preview-panel">
			<div data-testid="preview-content">{JSON.stringify(content)}</div>
			<button onClick={() => onApply(content)} disabled={isApplying}>
				Apply
			</button>
			<button onClick={onCancel}>Cancel</button>
			{provider && <span data-testid="preview-provider">{provider}</span>}
		</div>
	),
}));
...
describe('AiGeneratorPanel Component', () => {
	...
	describe('Requirement 7.1: Sidebar panel rendering', () => {
		...
		it('should display generation buttons initially', () => {
			render(<AiGeneratorPanel />);
			expect(screen.getByText('Generate All SEO')).toBeInTheDocument();
	...
	describe('Requirement 7.3 & 7.8: Provider indicator', () => {
		it('should display provider badge after successful generation', async () => {
			...
			await waitFor(() => {
				...
				expect(screen.getByTestId('preview-provider')).toHaveTextContent('OpenAI');
			});
		});
	});
	...
	describe('Requirement 10.1-10.4: Fallback notification', () => {
		it('should display fallback warning when fallback provider is used', async () => {
			...
			await waitFor(() => {
				...
				expect(notice).toHaveTextContent('primary provider unavailable');
		...
		it('should include link to settings in fallback notification', async () => {
			...
			await waitFor(() => {
				...
				expect(settingsLink).toHaveAttribute('href', 'http://example.com/settings');
			});
		});
	});
	...
	describe('Requirement 11.1-11.5: Error handling', () => {
		it('should display error message when generation fails', async () => {
			...
			await waitFor(() => {
				...
				expect(errorRegion.textContent).toContain('API key is invalid');
			});
		});
		...
		it('should handle permission denied error', async () => {
			...
			await waitFor(() => {
				const errorRegion = document.querySelector('[role="alert"][aria-live="assertive"]');
				expect(errorRegion.textContent).toContain('You do not have permission');
			});
		});
		...
		it('should handle content too short error', async () => {
			...
			await waitFor(() => {
				const errorRegion = document.querySelector('[role="alert"][aria-live="assertive"]');
				expect(errorRegion.textContent).toContain('at least 300 words');
			});
		});
	});
	...
	describe('Requirement 8.1-8.7: Preview panel display', () => {
		...
		it('should hide generation buttons when preview is displayed', async () => {
			...
			await waitFor(() => {
				expect(screen.queryByText('Generate All SEO')).not.toBeInTheDocument();
			});
		});
		...
		it('should pass generated content to preview panel', async () => {
			...
			await waitFor(() => {
				...
				expect(previewContent).toHaveTextContent(JSON.stringify(generatedData));
			});
		});
	});
	...
	describe('Requirement 27.1-27.10: Apply functionality', () => {
		...
		it('should display success message after applying content', async () => {
			...
			await waitFor(() => {
				...
				expect(screen.getByText('Content applied successfully!')).toBeInTheDocument();
			});
		});
		...
		it('should handle apply errors', async () => {
			...
			await waitFor(() => {
				const errorRegion = document.querySelector('[role="alert"][aria-live="assertive"]');
				expect(errorRegion.textContent).toContain('Failed to save content');
			});
		});
	...
	describe('Requirement 34.1-34.6: Accessibility features', () => {
		it('should have ARIA labels on all buttons', () => {
			...
			expect(generateAllButton.getAttribute('aria-label')).toContain('Generate all SEO');
		...
		it('should announce generation start to screen readers', async () => {
			...
			const statusRegion = document.querySelector('[role="status"][aria-live="polite"]');
			await waitFor(() => {
				expect(statusRegion.textContent).toContain('Generating content');
			});
		});
		...
		it('should announce generation success to screen readers', async () => {
			...
			const statusRegion = document.querySelector('[role="status"][aria-live="polite"]');
			await waitFor(() => {
				expect(statusRegion.textContent).toContain('generated successfully');
			});
		});
		...
		it('should announce errors to screen readers', async () => {
			...
			const alertRegion = document.querySelector('[role="alert"][aria-live="assertive"]');
			await waitFor(() => {
				expect(alertRegion.textContent).toContain('Generation failed');
			});
		});
