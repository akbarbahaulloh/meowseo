...
jest.mock('../ContentScoreWidget', () => ({
  ContentScoreWidget: () => {
    ...
    const { seoScore, readabilityScore, isAnalyzing } = useSelect((select: any) => ({
  ...
  TabContent: () => {
    ...
    return (
      <div data-testid="tab-content">
        {activeTab === 'general' && <div data-testid="general-content">General Content</div>}
        {activeTab === 'social' && <div data-testid="social-content">Social Content</div>}
        {activeTab === 'schema' && <div data-testid="schema-content">Schema Content</div>}
        {activeTab === 'advanced' && <div data-testid="advanced-content">Advanced Content</div>}
      </div>
    );
  },
}));
...
describe('Sidebar - Integration Tests', () => {
      ...
      const mockSelect = (storeName: string) => {
        if (storeName === 'meowseo/data') {
          return {
            ...
            getReadabilityScore: () => mockStoreState.readabilityScore,
            getAnalysisResults: () => mockStoreState.analysisResults,
            getActiveTab: () => mockStoreState.activeTab,
            getIsAnalyzing: () => mockStoreState.isAnalyzing,
            getContentSnapshot: () => mockStoreState.contentSnapshot,
          };
        }

        if (storeName === 'core/editor') {
          return {
            getEditedPostAttribute: (attr: string) => mockStoreState.contentSnapshot[attr] || '',
            getCurrentPostType: () => mockStoreState.contentSnapshot.postType,
            getCurrentPostId: () => 1,
            getPermalink: () => mockStoreState.contentSnapshot.permalink,
          };
        }
  ...
  it('should render sidebar with all main components', () => {
    ...
    expect(screen.getByTestId('content-score-widget')).toBeInTheDocument();
  ...
  it('should switch tabs correctly', () => {
    ...
    expect(screen.getByTestId('general-content')).toBeInTheDocument();
    expect(screen.queryByTestId('social-content')).not.toBeInTheDocument();
  ...
  it('should handle complete workflow: type, analyze, switch tabs', async () => {
    ...
    expect(screen.getByTestId('meowseo-sidebar')).toBeInTheDocument();
    expect(screen.getAllByTestId('content-score-widget').length).toBeGreaterThan(0);
    expect(screen.getByTestId('tab-bar')).toBeInTheDocument();
    expect(screen.getByTestId('tab-content')).toBeInTheDocument();
