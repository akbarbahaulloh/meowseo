...
describe( 'RobotsToggles', () => {
	...
	it( 'should display toggles as unchecked when values are empty', () => {
		...
		const noindexToggle = screen.getByLabelText(
			/no index/i
		) as HTMLInputElement;
		const nofollowToggle = screen.getByLabelText(
			/no follow/i
		) as HTMLInputElement;
	...
	it( 'should display help text for both toggles', () => {
		...
		const noindexHelp = screen.getByText(
			/prevent search engines from indexing/i
		);
		const nofollowHelp = screen.getByText(
			/prevent search engines from following links/i
		);
