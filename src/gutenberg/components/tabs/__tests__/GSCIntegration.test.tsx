...
describe('GSCIntegration', () => {
  ...
  it('should display last submission timestamp', () => {
    mockUseEntityPropBinding.mockReturnValue(['2024-01-15T10:30:00', mockSetLastSubmit]);
  ...
  it('should display Request Indexing button', () => {
    ...
    const button = screen.getByRole('button', { name: /request indexing/i });
  ...
  it('should display success message on successful request', async () => {
    ...
    await waitFor(() => {
      const successMessage = screen.getByText(/indexing request submitted successfully/i);
  ...
  it('should display help text mentioning manage_options capability', () => {
    ...
    const helpText = screen.getByText(/requires manage_options capability/i);
  ...
  it('should use useEntityPropBinding with _meowseo_gsc_last_submit', () => {
    ...
    expect(mockUseEntityPropBinding).toHaveBeenCalledWith('_meowseo_gsc_last_submit');
  });
