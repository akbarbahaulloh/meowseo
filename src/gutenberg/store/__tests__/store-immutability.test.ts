...
import { MeowSEOState, TabType, ContentSnapshot, AnalysisResult } from '../types';
...
const actionArbitrary: fc.Arbitrary<MeowSEOAction> = fc.oneof(
  contentSnapshotArbitrary.map((snapshot) => updateContentSnapshot(snapshot)),
  ...
  fc
    ...
    .map(([seoScore, readabilityScore, results]) =>
      setAnalysisResults(seoScore, readabilityScore, results)
    ),
...
describe('Store State Immutability Property Test', () => {
  it('should never mutate existing state objects when dispatching actions', () => {
    fc.assert(
      fc.property(fc.array(actionArbitrary, { minLength: 1, maxLength: 20 }), (actions) => {
        ...
        for (const action of actions) {
          ...
          expect(previousState).toEqual(stateReferences[stateReferences.length - 1]);
  ...
  it('should create new state objects for each action type', () => {
    fc.assert(
      fc.property(
        ...
        tabArbitrary,
        (snapshot, isAnalyzing, seoScore, readabilityScore, results, tab) => {
          const state1 = reducer(initialState, updateContentSnapshot(snapshot));
          expect(state1).not.toBe(initialState);
          expect(state1.contentSnapshot).toBe(snapshot);

          const state2 = reducer(state1, setAnalyzing(isAnalyzing));
          expect(state2).not.toBe(state1);
          expect(state2.isAnalyzing).toBe(isAnalyzing);

          const state3 = reducer(state2, setAnalysisResults(seoScore, readabilityScore, results));
          expect(state3).not.toBe(state2);
          expect(state3.seoScore).toBe(seoScore);
          expect(state3.readabilityScore).toBe(readabilityScore);
          expect(state3.analysisResults).toBe(results);
    ...
    fc.assert(
      fc.property(
        ...
        (snapshot, isAnalyzing, tab) => {
          ...
          const newState = reducer(state, updateContentSnapshot(snapshot));

          // Verify unaffected properties remain the same
          expect(newState.seoScore).toBe(seoScoreBefore);
          expect(newState.readabilityScore).toBe(readabilityScoreBefore);
