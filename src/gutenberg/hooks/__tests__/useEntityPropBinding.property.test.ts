...
describe( 'useEntityPropBinding - Property Tests', () => {
				...
				( metaUpdates ) => {
					...
					metaUpdates.forEach( ( update ) => {
						...
						const { result } = renderHook( () =>
							useEntityPropBinding( update.key )
						);
						...
						const lastCall =
							mockSetMeta.mock.calls[
								mockSetMeta.mock.calls.length - 1
							];
						...
						const hasCorrectValue =
							updatedMeta[ update.key ] === update.value;
	...
	it( 'should preserve other meta keys when updating one key', () => {
		fc.assert(
			fc.property(
				fc.record( {
					existingKey1: fc.constantFrom(
						'_meowseo_title',
						'_meowseo_description'
					),
					existingValue1: fc.string( {
						minLength: 1,
						maxLength: 50,
					} ),
					existingKey2: fc.constantFrom(
						'_meowseo_focus_keyword',
						'_meowseo_canonical'
					),
					existingValue2: fc.string( {
						minLength: 1,
						maxLength: 50,
					} ),
					updateKey: fc.constantFrom(
						'_meowseo_og_title',
						'_meowseo_twitter_title'
					),
				...
				( data ) => {
					...
					const { result } = renderHook( () =>
						useEntityPropBinding( data.updateKey )
					);
					const [ , setValue ] = result.current;
					...
					const hasExistingKey1 =
						updatedMeta[ data.existingKey1 ] ===
						data.existingValue1;
					const hasExistingKey2 =
						updatedMeta[ data.existingKey2 ] ===
						data.existingValue2;
					const hasUpdateKey =
						updatedMeta[ data.updateKey ] === data.updateValue;
