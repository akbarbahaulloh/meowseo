  ...
  it('should dispatch only once for rapid changes within 800ms', () => {
    fc.assert(
      fc.property(
        ...
        fc.array(fc.integer({ min: 10, max: 700 }), { minLength: 1, maxLength: 9 }),
        (contentChanges, delays) => {
          ...
          const dispatchCountBeforeDebounce = mockUpdateContentSnapshot.mock.calls.length;

          // Now advance past the 800ms debounce
          jest.advanceTimersByTime(800);

          // Should have dispatched exactly once
          const dispatchCountAfterDebounce = mockUpdateContentSnapshot.mock.calls.length;

          // Verify: only one dispatch after the debounce period
          return dispatchCountAfterDebounce === dispatchCountBeforeDebounce + 1;
        }
      ),
      {
      ...
      fc.property(
        ...
        (contentChanges) => {
          ...
          contentChanges.forEach((change) => {
            ...
            const actualDispatches = mockUpdateContentSnapshot.mock.calls.length;
        ...
        (numChanges, delay) => {
          ...
          for (let i = 0; i < numChanges; i++) {
            mockEditorSelect.getEditedPostAttribute.mockImplementation((attr: string) => {
              ...
              if (attr === 'content') return `Content ${i}`;
              return '';
            });
          ...
          const dispatchCountDuringChanges = mockUpdateContentSnapshot.mock.calls.length;

          // Now wait for the full 800ms
          jest.advanceTimersByTime(800);

          // Should have exactly one dispatch
          const finalDispatchCount = mockUpdateContentSnapshot.mock.calls.length;
