...
export default function LogViewer() {
	 ...
	 */
	useEffect(() => {
		sessionStorage.setItem('meowseo_log_filters', JSON.stringify(filters));
	}, [filters]);
	...
	const fetchLogs = async () => {
		...
		try {
			...
			const modules = [...new Set(response.logs.map(log => log.module))];
			setAvailableModules([
				{ label: __('All Modules', 'meowseo'), value: '' },
	...
	const handleBulkDelete = async () => {
		...
		try {
			...
			setSuccess(__('Successfully deleted selected log entries', 'meowseo'));
			setSelectedIds([]);
			...
			setError(err.message || __('Failed to delete log entries', 'meowseo'));
	...
	const handleCopyForAI = async () => {
		...
		try {
			...
			formatted += `- Plugin Version: ${window.meowseoLogViewer?.pluginVersion || 'Unknown'}\n`;
			formatted += `- WordPress Version: ${window.meowseoLogViewer?.wpVersion || 'Unknown'}\n`;
			formatted += `- PHP Version: ${window.meowseoLogViewer?.phpVersion || 'Unknown'}\n`;
			formatted += `- Active Modules: ${window.meowseoLogViewer?.activeModules || 'Unknown'}\n\n`;
			formatted += "## Log Entries\n\n";

			selectedLogs.forEach((log, index) => {
				formatted += `### Entry ${index + 1}: ${log.level} - ${log.module} Module\n`;
			...
			setError(err.message || __('Failed to copy log entries', 'meowseo'));
	...
	return (
		<div className="meowseo-log-viewer">
			...
			<Card>
				<CardBody>
					<Heading level={3}>{__('Filters', 'meowseo')}</Heading>
					<Flex gap={4} wrap>
				...
				<Card style={{ marginTop: '16px' }}>
					<CardBody>
						<Flex gap={2} align="center">
							<FlexItem>
								<Text>
									{__('Selected:', 'meowseo')} {selectedIds.length}
								</Text>
							</FlexItem>
							<FlexItem>
								<Button
						...
						<table className="wp-list-table widefat fixed striped">
							<thead>
								<tr>
									...
									<th style={{ width: '100px' }}>{__('Level', 'meowseo')}</th>
									<th style={{ width: '120px' }}>{__('Module', 'meowseo')}</th>
									<th>{__('Message', 'meowseo')}</th>
									<th style={{ width: '180px' }}>{__('Timestamp', 'meowseo')}</th>
									<th style={{ width: '80px' }}>{__('Hit Count', 'meowseo')}</th>
								</tr>
							</thead>
							<tbody>
								{logs.map((log) => (
									<>
										<tr key={log.id}>
											...
											<td style={{ textAlign: 'center' }}>{log.hit_count}</td>
										</tr>
										{expandedRows.includes(log.id) && (
											<tr key={`${log.id}-expanded`}>
												<td colSpan="7" style={{ backgroundColor: '#f9f9f9', padding: '16px' }}>
													{log.context && (
														<div style={{ marginBottom: '16px' }}>
															<strong>{__('Context:', 'meowseo')}</strong>
															<pre style={{
																backgroundColor: '#fff',
																padding: '12px',
													...
													{log.stack_trace && (
														<div>
															<strong>{__('Stack Trace:', 'meowseo')}</strong>
															<pre style={{
																backgroundColor: '#fff',
																padding: '12px',
																border: '1px solid #ddd',
				...
				<Card style={{ marginTop: '16px' }}>
					<CardBody>
						<Flex gap={4} align="center" justify="space-between">
							<FlexItem>
								<Text>
									{__('Showing', 'meowseo')} {((pagination.page - 1) * pagination.perPage) + 1} - {Math.min(pagination.page * pagination.perPage, pagination.total)} {__('of', 'meowseo')} {pagination.total} {__('entries', 'meowseo')}
								</Text>
							</FlexItem>
							<FlexItem>
								<Flex gap={2}>
									...
									<Text style={{ padding: '6px 12px' }}>
										{__('Page', 'meowseo')} {pagination.page} {__('of', 'meowseo')} {pagination.pages}
									</Text>
									<Button
										variant="secondary"
										onClick={() => handlePageChange(pagination.page + 1)}
			...
			{showDeleteModal && (
				...
				>
					<Text>
						{__('Are you sure you want to delete', 'meowseo')} {selectedIds.length} {__('log entries? This action cannot be undone.', 'meowseo')}
					</Text>
